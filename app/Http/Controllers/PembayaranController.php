...
class PembayaranController extends Controller
{
    ...
    public function createInvoice(Request $request)
    {
        try {
            ...
            if (!$existingPembayaran) {
                ...
                $items = ($data->jenis_kelamin === 'L') ? $this->getMaleItems() : $this->getFemaleItems();
                ...
                $createInvoiceRequest = $this->prepareInvoiceRequest($total, $items, $invoiceCustomerData, $notificationPreference, $url);
    ...
    private function getMaleItems()
    {
        return [
            ['name' => 'Satu Stel Dasar Pakaian Putih Dongker', 'price' => 140000, 'quantity' => 1],
            // Add other items for males here
        ];
    }

    private function getFemaleItems()
    {
        return [
            ['name' => 'Satu Stel Dasar Pakaian Putih Dongker', 'price' => 145000, 'quantity' => 1],
            // Add other items for females here
        ];
    }

    private function prepareInvoiceRequest($total, $items, $invoiceCustomerData, $notificationPreference, $url)
    {
